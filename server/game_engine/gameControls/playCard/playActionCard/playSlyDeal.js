...
const pickFieldCardToSlyDeal = ({Game, player, targetPlayer, forced, callback}) => {
  userControls.pickFieldCard({
    ...
    callback: ({error, card, source, cancelled, forced}) => {
      if (error) {
        ...
        processSlyDeal({Game, player, targetPlayer, card, source, forced, callback});
      }
    }
  });
}

const processSlyDeal = ({Game, player, targetPlayer, card, source, forced, callback}) => {
  if ('propertyCards' === source.pileName) {
    ...
    gameActions.moveCard({source: source.pile, destination: player.field[source.pileName], card});
    callback({card});
  }
};

const forcedPickFieldCardToSlyDeal = ({Game, player, targetPlayer, callback}) => {
  ...
  if (0 !== targetPlayer.field['buildingCards'].length) {
    ...
    for (let i = 0; i < size; i++) {
      if (gameActions.getPropertySetStatus({Game, destination: targetPlayer.field['propertyCards'][i]})) {
