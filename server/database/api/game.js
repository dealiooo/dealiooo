const getUserIdsAndUsernames = db => id =>
  db.th_games.findByPk(id).then(game => game.getUsers({ attributes: ['id', 'username'] }));
...
const endGame = db => id => db.th_games.update({ status: 'ended' }, { where: { id } });

const getStatus = db => id => db.th_games.findByPk(id).then(game => game.status);
