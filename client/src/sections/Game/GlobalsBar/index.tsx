...
const GlobalsBar: React.FC<GlobalsBarProps> = ({ gameGlobals, turnMessage, height }) => (
  <Container height={height}>
    ...
    <div>{turnMessage}</div>
    <RightContent>
      <Tooltip placement="bottom" trigger="hover" tooltip={<Detail>amount of cards left in deck</Detail>}>
        <Deck>
          <DeckIcon size={20} />
          <Strong as={CenterInline}>{gameGlobals.deckCount}</Strong>
        </Deck>
      </Tooltip>
      <Tooltip placement="bottom" trigger="hover" tooltip={<Detail>amount of cards in discard pile</Detail>}>
        <Discard>
          <DeckIcon size={20} />
          <Strong as={CenterInline}>{gameGlobals.discard.length}</Strong>
        </Discard>
      </Tooltip>
      <Tooltip placement="bottom" trigger="hover" tooltip={<Detail>total amount of turns</Detail>}>
        <TurnCount>
          <TurnsIcon size={20} />
