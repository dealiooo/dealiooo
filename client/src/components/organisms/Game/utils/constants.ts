import { Card } from '../../../../types/dealiooo';
...
export const calcGameGlobalsBarHeight = (screenHeight: number): number => Math.floor(screenHeight * 0.1);
...
export const deriveDescriptionFromCard = (card: Card): string => {
  switch (card.type) {
    case 'action': {
      const ActionCardDescriptionsMap = {
      ...
      return ActionCardDescriptionsMap[card.name];
    }
    case 'rent': {
